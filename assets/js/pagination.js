...
function renderPokedex(arr) {
    ...
    arr.map((item, index) => {
        ...
        if (index >= start && index < end) {
            template += `<div class="pokemon__item" >
    ...
    listPokemon.innerHTML = template;
}

function paginationPage() {
    let indexPage = ' <span class="active-pagination"></span>';
    for (let i = 1; i < totalPage; i++) {
        indexPage += '<span></span>';
    }
    let paginationPage = document.querySelector('.pagination-pokedex');
    paginationPage.innerHTML = indexPage;
    console.log(indexPage);
}

function changesPage() {
    let activePage = document.querySelector('.active-pagination');
    let page = document.querySelectorAll('span');

    activePage.classList.remove('active-pagination');
    page.classList.add('active-pagination');
